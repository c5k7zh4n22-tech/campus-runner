  ...
  return (
    <>
      ...
      <section className="mt-7">
        ...
        {queue.length ? <div className="grid gap-4 lg:grid-cols-2">{queue.map((user) => <article key={user.id} className="card p-5"><div className="flex items-start justify-between gap-4"><div><div className="font-black">{user.display_name}</div><div className="mt-1 text-xs text-slate-400">12 位学号：{user.student_id || "未填写"} · 手机号：{user.phone || "未填写"}</div></div><span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-bold text-amber-700">待审核</span></div><div className="mt-4 grid grid-cols-2 gap-2"><ActionForm action={reviewVerificationAction}><input type="hidden" name="userId" value={user.id} /><input type="hidden" name="status" value="verified" /><SubmitButton className="w-full" variant="primary">通过认证</SubmitButton></ActionForm><ActionForm action={reviewVerificationAction}><input type="hidden" name="userId" value={user.id} /><input type="hidden" name="status" value="rejected" /><SubmitButton className="w-full" variant="outline">拒绝</SubmitButton></ActionForm></div></article>)}</div> : <EmptyState title="暂无待审核认证" description="新的校园认证申请会出现在这里。" icon={ShieldCheck} />}
      </section>

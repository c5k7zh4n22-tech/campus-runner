...
const errorMessages: Record<string, string> = {
  wechat_not_configured: "微信登录暂未开放，请使用邮箱密码登录。",
  wechat_state_invalid: "微信登录状态已失效，请重试。",
  wechat_token_failed: "微信授权失败，请重新扫码。",
...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  const [params, configured] = await Promise.all([searchParams, Promise.resolve(isWechatLoginConfigured())]);
    ...
    <div className="mx-auto flex min-h-[calc(100vh-8rem)] max-w-md items-center px-4 py-10">
      <div className="w-full">
        ...
        <div className="card p-6 sm:p-8">
          ...
          <p className="mt-2 text-sm text-slate-500">推荐使用微信快速登录，首次登录后请完善校园认证。</p>

          {errorMessage ? <p className="mt-5 rounded-xl bg-rose-50 px-3 py-2 text-sm text-rose-700">{errorMessage}</p> : null}

          <div className="mt-7">
            <WechatLoginButton configured={configured} next="/" />
          </div>

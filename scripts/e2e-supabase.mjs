...
try {
  console.log("1/9 Creating two confirmed production test users...");
  ...
  const { data: campus, error: campusError } = await admin.from("campuses").select("id").limit(1).single();
  if (campusError) throw campusError;

  const { error: profileError } = await admin.from("profiles").upsert([
    { id: publisher.id, display_name: "E2E 发布者", campus_id: campus.id, phone: "13800138000", student_id: "E2E-P-001", verification_status: "verified", status: "active", role: "user" },
    { id: runner.id, display_name: "E2E 跑腿员", campus_id: campus.id, phone: "13900139000", student_id: "E2E-R-001", verification_status: "verified", status: "active", role: "user" }
  ]);
  if (profileError) throw profileError;

  console.log("2/9 Publishing an order...");
  ...
  console.log("3/9 Verifying private contact and RLS isolation...");
  ...
  console.log("4/9 Testing atomic acceptance and duplicate prevention...");
  ...
  assert(contact?.[0]?.phone === "13900139000", "Participant contact lookup failed");

  console.log("5/9 Testing invalid reward and past deadline...");
  ...
  console.log("6/9 Testing full legal status flow...");
  ...
  console.log("7/9 Testing reviews and duplicate prevention...");
  ...
  console.log("8/9 Testing report creation and admin authorization...");
  ...
  const { error: makeAdminError } = await admin.from("profiles").update({ role: "admin" }).eq("id", publisher.id);
  ...
  console.log("9/9 Testing multi-user data persistence without overwrite...");
